...
const SolarSystem = () => {
  ...
  useEffect(() => {
    if (experienceStarted) {
      ...
      planets.forEach((planet) => {
        const sound =
          planet !== "sat"
            ? planetsRef.current[planet].children[0]
            : planetsRef.current[planet].children[1];
        if (sound?.play) {
          sound.play();
        }
      });
  ...
  useFrame(({ clock }, delta) => {
    planets.forEach((p) => {
      ...
      pRef.position.x =
        Math.sin((clock.elapsedTime + randomStart) * orbitSpeed) *
        getPosition(p)[0];
      pRef.position.z =
        Math.cos((clock.elapsedTime + randomStart) * orbitSpeed) *
        getPosition(p)[0];
    });
  ...
  return (
    <>
      ...
      />
      <group
        ref={setSolarSystemRef}
        position={systemData.genSystemPosition}
        rotation={systemData.genSystemRotation}
      >
        <Float
          floatIntensity={0.5}
          floatingRange={0.25}
        ...
        >
          <group>
            ...
            {systemData.genAmbientLightEnabled && (
              <ambientLight intensity={systemData.genAmbientLightIntensity} />
            )}
...
const PlanetRings = forwardRef(
  ({ uBaseColor, uBloomIntensity, uRadiusInner, ...props }, ref) => {
    ...
    useFrame(({ clock }) => {
      ...
      ref.current.rotation.z =
        Math.atan2(distX, distZ) + Math.PI - solarSystemRef.rotation.y;
    });
    ...
    <>
      <EffectComposer multisampling={0} disableNormalPass={true}>
        ...
        {postConfig.vignette && (
          <Vignette
            eskil={false}
            offset={postConfig.vignetteOffset}
            darkness={postConfig.vignetteDarkness}
          />
        )}
      ...
      {postConfig.fog && (
        <fog
          attach="fog"
          args={[postConfig.fogColor, postConfig.fogNear, postConfig.fogFar]}
        />
      )}
    </>
  );
...
const WrappedOrbitControls = () => {
  ...
  const presetIsTransitioning = useStore(
    (state) => state.presetIsTransitioning
  );
