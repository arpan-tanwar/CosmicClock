...
export default function UI(props) {
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event) {
      if (
        settingsUIRef.current &&
        !settingsUIRef.current.contains(event.target)
      ) {
        setCurrentSettingsPanel(null);
      }
    }
  ...
  return (
    <div className="z-30">
      <div
        ref={settingsUIRef}
        className="absolute left-[26px] inset-y-0 flex items-center"
      >
        <div className="flex flex-col items-center gap-y-5">
          <button
            onClick={() => handleMenuClick("views")}
            aria-controls="views"
            aria-expanded={currentSettingsPanel === "views"}
          >
            <EyeIcon className="text-cloud-pink" />
          </button>
          <ViewSettings
            show={currentSettingsPanel === "views"}
            aria-controls="sounds"
            aria-expanded={currentSettingsPanel === "sounds"}
          />
      ...
      >
        <SpeakerIcon
          className={`w-[18px] ${
            muted ? "text-cloud-pink/50" : "text-cloud-pink"
          }`}
        />
      </button>
    </div>
